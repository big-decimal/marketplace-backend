...
public class CreateOrderUseCase {
	...
	public String apply(OrderCreateInput values) {
		...
		for (var cartItem : values.getCartItems()) {
			...
			if (pd != null) {
				var d = pd.getType() == Type.FIXED_AMOUNT ? pd.getValue()
						: orderItem.getUnitPrice().multiply(pd.getValue()).divide(BigDecimal.valueOf(100));
				if (d == null) {
					throw new ApplicationException("Unable to submit order");
				}
		...
		// var orderCode =
		// generateOrderCode(LocalDate.now(ZoneOffset.UTC).format(DateTimeFormatter.ofPattern("yyyyMMdd")));
	...
	private String generateOrderCode() {
		...
		do {
			...
			var ym = YearMonth.now().format(DateTimeFormatter.ofPattern("yyyyMM"));
			orderCode = ym + "_" + randomChars.toUpperCase();
		} while (orderDao.existsByCode(orderCode));
